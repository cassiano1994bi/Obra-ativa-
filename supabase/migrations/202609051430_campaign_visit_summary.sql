-- ObraAtiva: mostra ao proprietário visitas medidas mesmo quando o anúncio
-- ainda usa um link sem identificador de campanha. Não altera dados existentes.
begin;
...
begin
  ...
  return jsonb_build_object('settings',(select row_to_json(s) from public.product_insight_settings s where id),
    ...
    'summary',jsonb_build_object(
      'registered',(select count(*) from auth.users where created_at>=v_start),
      ...
      'visits',(select count(*) from public.product_campaign_visits where visited_at>=v_start),
      'visits_attributed',(select count(*) from public.product_campaign_visits where visited_at>=v_start and campaign_id is not null),
      'visits_unattributed',(select count(*) from public.product_campaign_visits where visited_at>=v_start and campaign_id is null)),
...
comment on function public.owner_insights_report(integer,text,integer,text) is
  'Relatório restrito ao proprietário, incluindo visitas medidas com e sem campanha identificada.';
