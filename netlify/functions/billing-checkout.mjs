...
export function createHandler(deps) {
  ...
  return endpoint(async request => {
    ...
    const protectStart = starts > b.now();
    ...
    const verifyStart = s => {
      if (protectStart && !(Date.parse(s.auto_recurring?.start_date) >= starts))
    ...
    try {
      ...
      const s = await b.mp('/preapproval', 'POST', {
        ...
        auto_recurring: { frequency: 1, frequency_type: 'months', transaction_amount: 69, currency_id: 'BRL',
          ...(protectStart ? { start_date: new Date(providerStarts).toISOString() } : {}) },
