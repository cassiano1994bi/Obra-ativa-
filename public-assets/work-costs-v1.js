(function () {
  ...
  if (typeof financeAttendanceLaborRows === 'function') { const before = financeAttendanceLaborRows; financeAttendanceLaborRows = function (...args) { return before(...args).map((row) => { const mode = C.attendanceMode(db, row.attendance); return { ...row, ...mode, value: mode.paymentMode === 'contract' ? 0 : row.value }; }); }; }
  ...
  if (typeof workCashRows === 'function') { const before = workCashRows; workCashRows = function () { return before().map((row) => { if (row.unassigned) return row; const cost = snapshot(row.work.id); return { ...row, ...cost, cash: cost.balanceAfterCosts, forecast: cost.estimated }; }); }; }
