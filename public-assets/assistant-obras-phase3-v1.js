(function installAssistantObraPhaseThree() {
  ...
  function decorate() {
    ...
    const navigationOwner = window.AssistantObraPhase6 || window.AssistantObraPhase4;
    if (!navigationOwner?.syncNavigation) phase2.insertAdjacentHTML('beforebegin', navigationMarkup());
    phase2.insertAdjacentHTML('afterend', areaMarkup());
    if (navigationOwner?.syncNavigation) navigationOwner.syncNavigation();
    else phase2.hidden = state.mode === 'reports';
  }
  ...
  function switchMode(mode, navigationManaged = false) {
    const navigationOwner = window.AssistantObraPhase6 || window.AssistantObraPhase4;
    if (!navigationManaged && navigationOwner?.syncNavigation) return navigationOwner.switchMode(mode);
    state.mode = mode === 'reports' ? 'reports' : 'chat'; renderTop(); decorate(); if (state.mode === 'reports') loadOptions();
  }
