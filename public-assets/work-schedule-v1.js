(function () {
  ...
  function summaryMarkup(phases) {
    ...
    return `<section class="ws-summary" aria-label="Resumo do cronograma">
      <article><small>Agora</small><b>${h(summary.current?.name || (summary.total ? 'Nenhuma etapa iniciada' : 'Cadastre uma fase'))}</b><span>${current ? h(current.label) : '—'}</span></article>
  ...
  function phaseScheduleMarkup(phase, phases) {
    ...
    const duration = info.durationDays ? `${info.durationDays} dia${info.durationDays === 1 ? '' : 's'} previstos` : info.remainingLabel;
