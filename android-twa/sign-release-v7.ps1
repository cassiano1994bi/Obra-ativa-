...
$releaseDir = Join-Path $projectRoot 'release-android-20260906-v7-obraativa'
...
$alignedApk = Join-Path $releaseDir 'obraativa-v7-aligned.apk'
$signedApk = Join-Path $releaseDir 'obraativa-v7.apk'
$signedAab = Join-Path $releaseDir 'obraativa-v7.aab'

foreach ($requiredPath in @($zipalign, $apksigner, $keystore, $passwordFile, $unsignedApk, $sourceAab)) {
